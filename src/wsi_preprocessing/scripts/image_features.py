...
from src.wsi_preprocessing.functions.path_io import get_path_up_to, extract_index

ROOT_DIR = get_path_up_to(os.path.abspath(__file__), "repos")
TARGET_DIR = f"{ROOT_DIR}/data/3_extracted_features/EXC/"

augmented_files = os.listdir(TARGET_DIR + "augmentation_features/")

for f in augmented_files:
    if f.endswith('.npz'):
        print(f)
        f = np.load(TARGET_DIR + "augmentation_features/" + f)
        ...
        augmentation_name = f.split("dization_")[-1].split(".")[0]

        # Save the features
        df_features.to_csv(f"{TARGET_DIR}/byol_features_{augmentation_name}.csv", index=False)

        print(f'Features saved to {TARGET_DIR}/byol_features_{augmentation_name}.csv')
