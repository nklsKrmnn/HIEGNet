...
def set_scale(ax, image):

    ticks = [0, 100, 200, 300, 400, 500]

    resolution = 0.252
    # Set X-axis ticks to micrometers instead of pixels
    x_ticks = [tick / resolution for tick in ticks] # Remove ticks outside the image
    ax.set_xticks(x_ticks)
    ax.set_xticklabels([f"{tick}" for tick in ticks], size=16)  # Convert to µm

    # Set labels
    ax.set_xlabel("µm", size=16)

    # Hide Y-axis
    ax.yaxis.set_visible(False)

# Define figure for composition of 4 images
ax_collection = []
...
size = image_og.shape[0]
max_d = 2*size

# shade image
image = cv2.addWeighted(image_og, 0.5, np.ones_like(image_og)*255, 0.5, 0)
...
legend_labels = {
    "Dead Glomeruli": "red",
    "Sclerotic Glomeruli ": "yellow",
    "Healthy Glomeruli": "green"
}
# Create a list of patches for the legend
legend_patches = [mpatches.Patch(color=color, label=label, alpha=0.5) for label, color in legend_labels.items()]
ax.legend(handles=legend_patches, loc="upper right", fontsize=16, frameon=True)

ax.imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
set_scale(ax, image)
ax_collection.append(ax)
plt.savefig("1_glom_masks.png",bbox_inches='tight')
...
ax_collection.append(ax)
plt.savefig("2_immune_masks.png",bbox_inches='tight')
...
nx.draw_networkx_edges(G, pos, alpha=0.25, ax=ax, edge_color='black', hide_ticks=False)
...
###### Safe iamge with immune cell masks ######
plt.savefig("3_c2c.png",bbox_inches='tight')

######################################### Add edges to glomeruli #########################################
# shade image
image = cv2.addWeighted(image_og, 0.8, np.ones_like(image_og)*255, 0.5, 0)
...
fig, ax = plt.subplots(1, 1, figsize=(10, 10))
ax.imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
nx.draw_networkx_edges(G, pos, edge_color=edge_colors, alpha=0.5, ax=ax, hide_ticks=False)

# Add legend
legend_labels = {
    "M0 to glomeruli": cm.Blues_r,
    "T-cell to glomeruli": cm.Purples_r
}
...
###### Safe iamge with immune cell masks ######
plt.savefig("4_c2g.png",bbox_inches='tight')

################################################3 Add glom2glom edges ############################################
# Get the edge attributes
edge_types = nx.get_edge_attributes(G, 'edge_type')
edge_weights = nx.get_edge_attributes(G, 'edge_weight')
...
# Define color palettes for other edge types
type_colormaps = {
    ('tcell', 'to','glom'): cm.Greys_r,
    ('M0', 'to', 'glom'): cm.Greys_r
}
...
fig, ax = plt.subplots(1, 1, figsize=(10, 10))
ax.imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
nx.draw_networkx_edges(G, pos, edge_color=edge_colors, alpha=0.4, ax=ax, hide_ticks=False)
...
nx.draw_networkx_edges(G3, pos, edge_color='orange', alpha=1, ax=ax, width=2, hide_ticks=False)
...
###### Safe iamge with immune cell masks ######
plt.savefig("5_g2g.png",bbox_inches='tight')


####### Safe composition of all images ######
fig, axs = plt.subplots(1, 2, figsize=(10, 10))

axs[0] = ax_collection[0]
axs[0] = ax_collection[1]

plt.savefig("6_composition.png",bbox_inches='tight')